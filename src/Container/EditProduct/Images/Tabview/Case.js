...
const ImageData = (props) => {
    if (props.product_details.hasOwnProperty('case_image')) {
        let images = props.product_details.case_image;
        if (images.length) {
            return <ImageView {...props.product_details} imageType='case' />
        } else {
            return (
                <div className="col-sm-6 col-md-10 col-md-offset-1">
                    <div className="upload_image text-center">
                        <AlertDismisable message={props.case_image_type_error} color='danger' />
                        <div className="form-group image_upload upload_assets">
                            <FileBase64 multiple={true} onDone={props.getFiles.bind(this, 'case')} />
                            <div id="image_preview" className={(props.case_image_preview) ? '' : 'hide'}>
                                {props.case_image_preview}
                            </div>
                        </div>
                    </div>
                </div>
